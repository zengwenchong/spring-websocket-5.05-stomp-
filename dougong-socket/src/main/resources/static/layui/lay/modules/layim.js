...
layui.define(["layer", "laytpl", "upload"], function(i) {
	...
	layui.data.date = function(i) {
		...
		return a.getFullYear() + "-" + h(a.getMonth() + 1) + "-" + h(a.getDate()) + " " + h(a.getHours()) + ":" + h(a.getMinutes()) + ":" + h(a.getSeconds())
	...
	var v, g, x, b, w, k = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }}>', '<div class="layim-chat-user"><img src="{{ d.avatar }}"><cite>', "{{# if(d.mine){ }}", '<i>{{ layui.data.date(d.timestamp) }}</i>{{ d.username||"佚名" }}', "{{# } else { }}", '{{ d.username||"佚名" }}<i>{{ layui.data.date(d.timestamp) }}</i>', "{{# } }}", "</cite></div>", '<div class="layim-chat-text">{{ layui.data.content(d.content||"&nbsp") }}</div>', "</li>"].join(""),
		C = '<li class="layim-chatlist-{{ d.data.type }}{{ d.data.id }} layim-this" layim-event="tabChat"><img src="{{ d.data.avatar }}"><span>{{ d.data.name||"佚名" }}</span>{{# if(!d.base.brief){ }}<i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>',
		S = function(i, a, n) {
			return i = i || {}, e.ajax({
				url: i.url,
				type: i.type || "get",
		...
		L = function(i) {
			return t.open({
				...
				content: i,
				success: function(i) {
					v = i, _(i), j.base.right && i.css("margin-left", "-" + j.base.right), g && t.close(g.attr("times"));
					var a = [],
		...
		z = function() {
			v.on("contextmenu", function(i) {
				return i.cancelBubble = !0, i.returnValue = !1, !1
			});
			var i = function() {
				t.closeAll("tips")
			};
			v.find(".layim-list-history").on("contextmenu", "li", function(a) {
				var n = e(this),
		...
		I = function(i) {
			return g && t.close(g.attr("times")), v && v.hide(), j.mine = j.mine || {}, t.open({
				type: 1,
				title: !1,
				...
				success: function(i, a) {
					g = i, j.base.right && i.css("margin-left", "-" + j.base.right), i.on("click", function() {
						t.close(a), v.show();
		...
		T = function(i) {
			...
			if(a[0]) {
				var s = x.find(".layim-chat-list"),
					o = s.find(".layim-chatlist-" + i.type + i.id),
					r = x.find(".layui-layer-max").hasClass("layui-layer-maxmin"),
					u = a.children(".layim-chat-box");
				return "none" === x.css("display") && x.show(), b && t.close(b.attr("times")), 1 !== s.find("li").length || o[0] || (r || x.css("width", 800), s.css({
					height: x.height()
				}).show(), u.css("margin-left", "200px")), o[0] || (s.append(n(C).render(l)), u.append(n(f).render(l)), N()), M(s.find(".layim-chatlist-" + i.type + i.id)), o[0] || P(), R(i), Z(), w
			}
			l.first = !0;
			var c = w = t.open({
				type: 1,
				...
				content: n('<ul class="layui-unselect layim-chat-list">' + C + '</ul><div class="layim-chat-box">' + f + "</div>").render(l),
				success: function(a) {
					x = a, a.css({
						"min-width": "500px",
						"min-height": "420px"
					}), Z(), _(a), R(i), P(), q(), layui.each(d.chatChange, function(i, a) {
						a && a(O())
				...
				min: function() {
					...
					t.closeAll("tips"), x = null
				}
			});
			return c
		},
		N = function() {
			var i = x.find(".layim-chat-list"),
				a = x.find(".layim-chat-main"),
				e = x.height();
			i.css({
				height: e
			}), a.css({
		...
		A = function(i) {
			...
			x && !i && x.hide(), t.close(A.index), A.index = t.open({
				type: 1,
				title: !1,
		...
		M = function(i, a) {
			...
			var n = i.index() === -1 ? 0 : i.index(),
				l = ".layim-chat",
				r = x.find(l).eq(n),
				u = x.find(".layui-layer-max").hasClass("layui-layer-maxmin");
			if(a) {
				i.hasClass(o) && M(0 === n ? i.next() : i.prev()), i.remove(), r.remove();
				var c = x.find(l).length;
				return 1 === c && (x.find(".layim-chat-list").hide(), u || x.css("width", "600px"), x.find(".layim-chat-box").css("margin-left", 0)), 0 === c && t.close(w), !1
			}
		...
		R = function(i) {
			var a = layui.data("layim")[j.mine.id] || {},
				...
				l = t[i.type + i.id];
			if(v) {
				var s = v.find(".layim-list-history");
		...
		Y = function(i) {
			...
			var a = e(".layim-chatlist-" + i.type + i.id),
				t = {},
				l = a.index();
			if(i.timestamp = i.timestamp || (new Date).getTime(), i.system || B(i), K = JSON.parse(JSON.stringify(i)), j.base.voice && G(), !x && i.content || l === -1) {
			...
			if(x) {
				...
				var d = x.find(".layim-chat").eq(l),
					u = d.find(".layim-chat-main ul");
				i.system ? l !== -1 && u.append('<li class="layim-chat-system"><span>' + i.content + "</span></li>") : "" !== i.content.replace(/\s/g, "") && u.append(n(k).render(i)), W()
			}
		},
		$ = "layui-anim-loop layer-anim-05",
		F = function(i) {
			var a = v.find(".layim-tool-msgbox");
			a.find("span").addClass($).html(i)
		},
		B = function(i) {
			var a = layui.data("layim")[j.mine.id] || {},
				e = a.chatlog || {};
			e[i.type + i.id] ? (e[i.type + i.id].push(i), e[i.type + i.id].length > r && e[i.type + i.id].shift()) : e[i.type + i.id] = [i], a.chatlog = e, layui.data("layim", {
				key: j.mine.id,
				value: a
			})
		},
		P = function() {
			...
			layui.each(e[a.data.type + a.data.id], function(i, a) {
				t.append(n(k).render(a))
			}), W()
		},
		U = function(i) {
			var a, e = {},
				l = v.find(".layim-list-" + i.type);
			if(j[i.type])
		...
		V = function(i) {
			var a = v.find(".layim-list-" + i.type);
		...
		ei = {
			...
			sign: function() {
				var i = v.find(".layui-layim-remark");
			...
			tab: function(i) {
				var a, e = ".layim-tab-content",
					t = v.find(".layui-layim-tab>li");
				"number" == typeof i ? (a = i, i = t.eq(a)) : a = i.index(), a > 2 ? t.removeClass(o) : (ei.tab.index = a, i.addClass(o).siblings().removeClass(o)), v.find(e).eq(a).addClass(s).siblings(e).removeClass(s)
			},
			spread: function(i) {
			...
			search: function(i) {
				var a = v.find(".layui-layim-search"),
					e = v.find("#layui-layim-search"),
					t = a.find("input"),
					n = function(i) {
			...
			msgbox: function() {
				var i = v.find(".layim-tool-msgbox");
				...
				i.data("down") ? l() : (n.html("&#xe619;"), i.data("down", !0), ei.groupMembers.index = t.tips('<ul class="layim-members-list"></ul>', i, {
					...
					success: function(a) {
						var t = j.base.members || {},
							...
							r = {},
							u = x.find(".layui-layer-max").hasClass("layui-layer-maxmin"),
							c = "none" === x.find(".layim-chat-list").css("display");
						u && s.css({
			...
			extend: function(i) {
				var a = i.attr("lay-filter"),
					e = O();
				layui.each(d["tool(" + a + ")"], function(i, a) {
					a && a(function(i) {
						ii(e.textarea[0], i)
					})
				})
	...
	i("layim", new u)
}).addcss("modules/layim/layim.css?v=3.0Pro", "skinlayimcss");